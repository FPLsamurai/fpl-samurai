...
TEAM_JA = {
    ...
    "BOU": "ボーンマス", "BRE": "ブレントフォード", "CHE": "チェルシー",
    "COV": "コヴェントリー", "CRY": "クリスタルパレス", "EVE": "エヴァートン",
    "FUL": "フラム", "HUL": "ハルシティ", "IPS": "イプスウィッチ",
    "LEE": "リーズ", "LIV": "リヴァプール",
    "MCI": "マンシティ", "MUN": "マンユナイテッド",
    "NEW": "ニューカッスル", "NFO": "フォレスト",
    "SUN": "サンダーランド", "TOT": "トッテナム",
}
...
def compute_player_goal_ranking(bootstrap, fixtures, team_matches, team_map, mu,
    ...
    期待ゴール = 選手の直近5試合の平均xG × (相手の直近5試合の平均被xG ÷ リーグ平均xG(μ))
    ...
    def opp_factor(opp_id):
        # 相手の直近5試合（未満なら消化ぶん）の平均被xG ÷ リーグ平均xG
    ...
    for r in player_rows:
        ...
        xg_avg = r.get("xg", 0) / matches             # 直近5試合（未満なら消化ぶん）の平均xG
        expected = xg_avg * factor
        if expected <= 0:
            continue
        rows.append({
